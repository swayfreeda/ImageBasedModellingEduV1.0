...
generate_candidate(int label, TextureView const & texture_view,
    ...
    core::ByteImage::Ptr byte_image;
    byte_image = core::image::crop(view_image, width, height, min_x, min_y, *math::Vec3uc(255, 0, 255));
    core::FloatImage::Ptr image = core::image::byte_to_float_image(byte_image);
...
bool fill_hole(std::vector<std::size_t> const & hole, UniGraph const & graph,
    core::TriangleMesh::ConstPtr mesh, core::MeshInfo const & mesh_info,
    std::vector<std::vector<VertexProjectionInfo> > * vertex_projection_infos,
    ...
    for (std::size_t j = 0; j < num_vertices; ++j, ++it) {
        ...
        if (mesh_info[vertex_id].vclass != core::MeshInfo::VERTEX_CLASS_SIMPLE) {
    ...
    core::FloatImage::Ptr image = core::FloatImage::create(image_size, image_size, 3);
...
void
generate_texture_patches(UniGraph const & graph,
    ...
    std::vector<TexturePatch::Ptr> * texture_patches)
{
...

    {
        ...
        if (!unseen_faces.empty()) {
            core::FloatImage::Ptr image = core::FloatImage::create(3, 3, 3);
            std::vector<math::Vec2f> texcoords;
